...
public class DbQueryMs<T> implements IQuery<T> ,IDbQuery<T>{

	public DbQueryMs(String connKey, Class<?> type) {
		ConnKey = connKey;
		TEntity = new Entity<T>(type);
		trackSql = trackSql.replace("{table}", "["+TEntity.tableName+"]");
	}
	
	public DbQueryMs(String connKey, Class<?> type, String prefix) {
		ConnKey = connKey;
