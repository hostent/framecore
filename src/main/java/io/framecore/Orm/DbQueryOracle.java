...
public class DbQueryOracle<T> implements IQuery<T> ,IDbQuery<T>{
	...
	private String trackSql = "select {column} from {table} {where} {order}";
	...
	private void BuildColumns(String cols) {
		...
		if(selectExp != null && (!selectExp.isEmpty()))
		{
	...
	private void BuildLimit(String limit) {
		...
		if (limitLength == null||limitLength==0) {			 
			return;
		}
		...
		if(limitForm==0)
		{
			...
			trackSql = String.format(format, trackSql,limitLength);
		...
		else
		{
			...
			trackSql = String.format(format, trackSql,limitForm+limitLength,limitForm);
		}
