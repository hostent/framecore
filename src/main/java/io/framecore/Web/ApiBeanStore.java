...
import io.framecore.Frame.ApiMethod;

public class ApiBeanStore {

	public static Map<String, ApiBeanStore> StoreList = new ConcurrentHashMap<String,ApiBeanStore>();
	...
	public static void FillList(Map<String, Object> beans) {
		...
		for (String key : beans.keySet()) {
			...
			for (Method method : methods) {
				...
				int lockSeond = 0;
				try {
					lockSeond =Integer.valueOf(apiMethod.distributedLockSecond());
				} catch (Exception e) {	
					//undo
				}
				if(lockSeond>0)
				{
					store.setDistributedLockSecond(lockSeond);
				} 
				
				StoreList.put(apiMethod.value().toLowerCase(),store);
			}
	...
	private int distributedLockSecond=0;;
	...
	public int getDistributedLockSecond() {
		return distributedLockSecond;
	}

	public void setDistributedLockSecond(int distributedLockSecond) {
		this.distributedLockSecond = distributedLockSecond;
	}
